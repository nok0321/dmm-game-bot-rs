...
`BITMAPINFOHEADER.biHeight = -height_i32` で **トップダウン** ピクセル順を要求し、
`GetDIBits` の出力をそのまま `RgbaImage::from_raw` に渡せるようにしている。
リソースは GDI ハンドルを `DeleteObject` / `DeleteDC` / `ReleaseDC` で解放。

**i32 範囲ガード**: `width` / `height` (`u32`) はキャプチャ各関数の入口
(`capture_with_bitblt_from_screen` / `capture_with_print_window` / `extract_pixels`) で
共通ヘルパ `dim_to_i32` を介し `i32::try_from` で検証する。
`> i32::MAX` のときは `BotError::CaptureFailed("{label} {value} exceeds i32 range")` で早期にエラー化する。
このガードがないと `biHeight: -(height as i32)` で `u32 → i32` の wraparound 後に符号反転が
正方向に戻り、トップダウン指定が逆転する (BOTTOM-UP 解釈で上下反転画像) 可能性がある。
`CreateCompatibleBitmap` / `BitBlt` に渡す `width as i32` / `height as i32` も同様の
ガード経由 (`width_i32` / `height_i32`) を使う。
